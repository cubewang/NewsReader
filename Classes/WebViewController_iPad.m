...
@synthesize audioDownloader, player, audioPath,playButton,pauseButton;
...
@synthesize doesDownlodeAudioLabel;
@synthesize yesButton,noButton;
@synthesize backButton;
@synthesize translationButton,shareButton;
...
- (void)viewDidLoad {
    ...
    self.doesDownlodeAudioLabel.text = NSLocalizedString(@"是否下载音频到本地收听？",@"");
    [self.yesButton setTitle:NSLocalizedString(@"是",@"") forState:UIControlStateNormal];
    [self.noButton setTitle:NSLocalizedString(@"否",@"") forState:UIControlStateNormal];
    
    [self.backButton setTitle:NSLocalizedString(@"返回", @"") forState:UIControlStateNormal];
    
    [self.favoriteButton setTitleEdgeInsets:UIEdgeInsetsMake(20,-5,0,-5)];
    [self.favoriteButton setTitle:NSLocalizedString(@"收藏", @"") forState:UIControlStateNormal];
    
    [self.shareButton setTitleEdgeInsets:UIEdgeInsetsMake(20,-5,0,-5)];
    [self.shareButton setTitle:NSLocalizedString(@"分享", @"") forState:UIControlStateNormal];
    
    [self.translationButton setTitleEdgeInsets:UIEdgeInsetsMake(20,-10,0,-10)];
    [self.translationButton setTitle:NSLocalizedString(@"翻译verb", @"") forState:UIControlStateNormal];
    
    [self.audioButton setTitleEdgeInsets:UIEdgeInsetsMake(20,-5,0,-5)];
    [self.audioButton setTitle:NSLocalizedString(@"听阅", @"") forState:UIControlStateNormal];
...
- (void)resetControls
{
    ...
    [favoriteButton setBackgroundImage:[UIImage imageNamed:@"favorite_off_iPad.png"] forState:UIControlStateNormal];
...
- (void)loadArticle:(NSString *)articleId
{
    ...
    if ([article.CommentCount isEqualToString:@"0"]) {
       [commentBarItem setTitle: NSLocalizedString(@"暂无评论", @"") forState:UIControlStateNormal];
    }
    else if ([article.CommentCount length] == 0) {
        [commentBarItem setTitle: NSLocalizedString(@"查看评论", @"") forState:UIControlStateNormal];
        //commentBarItem.title = NSLocalizedString(@"查看评论", @"");
    }
    else {
         
        NSString  *titleStr = [NSString stringWithFormat:NSLocalizedString(@"%@条评论", @""), article.CommentCount];
        [commentBarItem setTitle: titleStr forState:UIControlStateNormal];
    }
}
...
- (void)customMenuAction:(id)sender
{
    if (_word)
    {
        WordCardViewController_iPad *vc = [[WordCardViewController_iPad alloc] initWithNibName:@"WordCardView_iPad"bundle:nil];
        vc.word = _word;
...
- (IBAction)favoriteButtonClicked:(UIButton *)sender
{
    ...
    {
        [favoriteButton setBackgroundImage:[UIImage imageNamed:@"favorite_on_iPad.png"] forState:UIControlStateNormal];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    ShareViewController_iPad *shareVC = [[ShareViewController_iPad alloc] initWithShareText:sharedText 
                                                                                   andImage:shareImage
                                                                                    andType:buttonIndex];
    
    [shareVC initWithNibName:@"ShareView_iPad" bundle:nil];
    
    shareVC.postImageURL = article.SourceImageUrl;
    shareVC.shareArticleName = article.Name;
    shareVC.article = article;
    
    shareVC.modalPresentationStyle = UIModalPresentationFormSheet;
...
- (IBAction)audioButtonClicked:(UIButton *)sender
{
    if (audioPlayingToolbar.hidden) {
        [audioPlayingToolbar setHidden:NO];         
        [audioButton setBackgroundImage: [UIImage imageNamed:@"listen_off_iPad"] forState:UIControlStateNormal];
    }
    else {
        [audioPlayingToolbar setHidden:YES];
        [audioButton setBackgroundImage: [UIImage imageNamed:@"listen_on_iPad"] forState:UIControlStateNormal];    }  
}
...
- (void)dealloc {
    ...
    self.doesDownlodeAudioLabel = nil;
