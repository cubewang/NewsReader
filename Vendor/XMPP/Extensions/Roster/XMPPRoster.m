...
- (id)initWithRosterStorage:(id <XMPPRosterStorage>)storage 
              xmppIQRequest:(XMPPIQRequest *)iqRequest
{
    xmppIQRequestModule = [iqRequest retain];
    return [self initWithRosterStorage:storage];
}
...
- (void)acceptBuddyRequest:(XMPPJID *)jid
{
    ...
    //dyg add we could not add roster in our list
    //just agree the request
...
- (BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq
{
    ...
    if (query)
    {
        ...
        if (![self hasRoster])
        {
            ...
            [multicastDelegate xmppRosterDidReceiveRosterList:self];
        }
