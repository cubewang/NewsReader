...
- (void)recalculatePrimaryResource
{
    ...
    {
        for (XMPPResourceCoreDataStorageObject *item in sortedResources) {
            if ([[item type] isEqualToString:@"available"])
            {
                self.primaryResource = item;
                bAvailable = YES;
            }
        }
    }
...
- (BOOL)isFriend //好友
{
    return [self.subscription isEqualToString:@"both"];
}

- (BOOL)isFollow //关注
{
    NSLog(@"%@", self.subscription);
    return [self.subscription isEqualToString:@"to"];
}

- (BOOL)isFans   //粉丝
{
    return [self.subscription isEqualToString:@"from"];
}

- (BOOL)isNone   //为定义
{
    return [self.subscription isEqualToString:@"none"];
}
