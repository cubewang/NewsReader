...
- (void)_clearAllUsersAndResourceForXMPPStream:(XMPPStream *)stream
{
    NSAssert(dispatch_get_current_queue() == storageQueue, @"Invoked on incorrect queue");
...
- (void)beginRosterPopulationForXMPPStream:(XMPPStream *)stream
{
	...
	[self scheduleBlock:^{
        ...
        [self _clearAllUsersAndResourceForXMPPStream:stream];
	}];
}
...
{
	XMPPLogTrace();
	XMPPPresence *newPresence = [presence copy];
	
	[self scheduleBlock:^{
		
		XMPPJID *jid = [newPresence from];
		XMPPUserCoreDataStorageObject *user = (XMPPUserCoreDataStorageObject *)[self _userForJID:jid xmppStream:stream];
		...
		if (user)
		{
			...
			[user updateWithPresence:newPresence streamBareJidStr:streamBareJidStr];
		}
