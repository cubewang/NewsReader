...
#define TIMEOUT_XMPP_BUILD_CONNECT 30
...
- (BOOL)connectToHost:(NSString *)host onPort:(UInt16)port error:(NSError **)errPtr
{
	...
	//dyg add the time out
	BOOL result = [asyncSocket connectToHost:host onPort:port withTimeout:TIMEOUT_XMPP_BUILD_CONNECT error:errPtr];
...
- (BOOL)connect:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{
		...
		if ([hostName length] == 0)
		{
			...
			[srvResolver startWithSRVName:srvName timeout:TIMEOUT_XMPP_BUILD_CONNECT];
		...
		if (state != STATE_XMPP_DISCONNECTED)
		{
			...
			if (state == STATE_XMPP_RESOLVING_SRV)
			{
				...
				state = STATE_XMPP_DISCONNECTED;
                		...
                		XMPPLogError(@"xmppStreamDidDisconnect because %@ called",
                           		THIS_METHOD);
...
//服务器已知，我们去掉了XMPP标准的部分流程
- (BOOL)registerWithPasswordEx:(NSString *)password error:(NSError **)errPtr
{
    XMPPLogTrace();
...
- (BOOL)authenticateWithPasswordEx:(NSString *)password error:(NSError **)errPtr
{
    XMPPLogTrace();
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err
{
	...
	else
	{
		...
		if (parserError)
		{
           	 	XMPPLogError(@"xmppStreamDidDisconnect because %@ called parserError ",
                         THIS_METHOD);
		...
		{
            		XMPPLogError(@"xmppStreamDidDisconnect because %@ called unknown",
                         THIS_METHOD);
